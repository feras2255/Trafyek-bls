...
export default async function About() {
  ...
  const stats = [
    { label: t("stats.projects"), val: "+250", icon: FiCheckCircle },
    { label: t("stats.partners"), val: "+220", icon: FiUsers },
    { label: t("stats.experience"), val: "+6", icon: FiStar },
    { label: t("stats.satisfaction"), val: "100%", icon: FiAward },
  ];

  const whyUsPoints = [
    { title: t("whyUs.point1Title"), desc: t("whyUs.point1Desc") },
    { title: t("whyUs.point2Title"), desc: t("whyUs.point2Desc") },
    { title: t("whyUs.point3Title"), desc: t("whyUs.point3Desc") },
    { title: t("whyUs.point4Title"), desc: t("whyUs.point4Desc") },
    { title: t("whyUs.point5Title"), desc: t("whyUs.point5Desc") },
  ];

  const features = [
    { t: t("features.f1Title"), d: t("features.f1Desc") },
    { t: t("features.f2Title"), d: t("features.f2Desc") },
    { t: t("features.f3Title"), d: t("features.f3Desc") },
    { t: t("features.f4Title"), d: t("features.f4Desc") },
  ];
      ...
      <div className="container mx-auto px-4 lg:px-6 mt-20 relative z-10 space-y-32">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-6">
            <span className="inline-block px-4 py-1.5 bg-primary/10 text-primary rounded-full text-xs font-black tracking-widest uppercase">
              {t("badge")}
            </span>
            <h1 className="text-3xl md:text-5xl font-black text-accent tracking-tight leading-tight">
              {t("title")}
            </h1>
            <p className="text-lg font-medium text-subtext leading-relaxed">
              {t("storyP1")}
            </p>
            <div className="p-4 bg-primary/10 border-r-4 border-primary rounded-lg text-primary font-black text-xl italic">
              {t("storyHighlight")}
            </div>
            <p className="text-lg font-medium text-subtext leading-relaxed">
              {t("storyP2")}
        ...
        {/* Vision & Mission */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          <div className="bg-accent text-text py-12 px-6 md:p-12 rounded-2xl md:rounded-[3.5rem] relative overflow-hidden shadow-xl space-y-4">
            <FiTarget className="text-primary size-16 mb-4 relative z-10" />
            <h3 className="text-3xl font-black relative z-10 italic">
              {t("vision.title")}
            </h3>
            <p className="text-text/80 leading-relaxed text-base md:text-lg font-medium relative z-10">
              {t("vision.description")}
            </p>
          </div>

          <div className="bg-card border border-slate-100 py-12 px-6 md:p-12 rounded-2xl md:rounded-[3.5rem] relative overflow-hidden shadow-sm space-y-4">
            <FiCompass className="text-primary size-16 mb-4" />
            <h3 className="text-3xl font-black text-accent italic">
              {t("mission.title")}
            </h3>
            <p className="text-subtext leading-relaxed text-base md:text-lg font-medium">
        ...
        {/* Why Us */}
        <div className="bg-card border border-slate-100 py-12 px-6 md:p-12 rounded-3xl md:rounded-[3.5rem] shadow-sm space-y-8">
          <div className="space-y-2">
            <h3 className="text-3xl font-black text-accent flex items-center gap-3">
              <FiHelpCircle className="text-primary" />
              {t("whyUs.title")}
            </h3>
            <p className="text-subtext font-bold text-lg">
              {t("whyUs.subtitle")}
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {whyUsPoints.map((pt, idx) => (
              <div
                key={idx}
                className="p-6 bg-background-2/50 rounded-2xl space-y-2 border border-slate-100"
              >
                <h4 className="font-black text-accent text-lg">{pt.title}</h4>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-8 gap-x-6">
            {features.map((item, idx) => (
              <div key={idx} className="space-y-2">
                ...
                <p
                  className={`text-subtext text-sm leading-relaxed ${isRtl ? "pr-6" : "pl-6"}`}
                >
                  {item.d}
                </p>
              </div>
            ...
            <div className="max-w-4xl mx-auto space-y-10 relative z-20">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-6 pt-6">
                <Link
                  ...
                  target="_blank"
                  rel="noreferrer"
                  aria-label={t("cta.startBtn")}
                  className="group relative w-full md:w-auto px-12 py-5 bg-primary text-text font-black text-xl rounded-2xl shadow-xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all duration-300 overflow-hidden"
                >
                  <span className="relative z-10">{t("cta.startBtn")}</span>
                  <div className="absolute inset-0 bg-white opacity-0 group-hover:opacity-20 transition-opacity" />
                </Link>
